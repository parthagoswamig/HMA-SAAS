...
function PatientForm({
  ...
  useEffect(() => {
    ...
    console.log('🔵 [DEBUG] Step name:', steps[activeStep]?.label);
    if (activeStep === 5) {
      console.log('✅ [DEBUG] Reached Review & Submit step');
      console.log('🔵 [DEBUG] Submit button should be visible now');
    }
  }, [activeStep]);
  ...
  const handleSubmit = async (values: CreatePatientDto) => {
    try {
      ...
      console.log('🔵 [DEBUG] Calling onSubmit with data...');
      await onSubmit(flattenedData);
      console.log('✅ [DEBUG] onSubmit completed successfully');

      // Close the modal after successful submission
      // Parent has already shown the notification
      handleClose();
    } catch (error: any) {
  ...
  const nextStep = () => {
    ...
    console.log('🔵 [DEBUG] Form values:', form.values);
    const currentStepValid = validateCurrentStep();
    console.log('🔵 [DEBUG] Step validation result:', currentStepValid);
    if (currentStepValid) {
      const nextStepIndex = activeStep + 1;
      console.log('✅ [DEBUG] Moving to next step:', nextStepIndex);
      setActiveStep((current) => (current < steps.length - 1 ? current + 1 : current));
    } else {
      console.log('❌ [DEBUG] Validation failed, staying on current step');
    }
  };
  ...
  const validateCurrentStep = (): boolean => {
    switch (activeStep) {
      case 0: // Basic Info
        ...
        return true;
      case 1: // Contact
        // Only require phone number - make everything else optional
        ...
        // Address fields are now optional - backend will handle defaults
        return true;
      case 2: // Medical
              ...
              <Card withBorder>
                ...
                <Stack gap="xs">
                  ...
                  <Group>
                    ...
                    <Text>
                      {form.values.address.street || 'Not provided'}
