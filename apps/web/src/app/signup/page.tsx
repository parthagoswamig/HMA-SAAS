...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use new tenant onboarding endpoint (Phase 6)
      ...
      const response = await apiFetch(`${API_BASE_URL}/tenant-onboarding`, {
        method: 'POST',
        headers: {
      ...
      setSuccess(`Account created successfully! Tenant: ${result.data.tenantSlug}. Redirecting to login...`);
      setTimeout(() => {
        router.push('/login');
      }, 2000);
    } catch (err) {
