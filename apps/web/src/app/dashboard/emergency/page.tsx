...
const EmergencyManagement = () => {
  ...
  const [bedAssignmentOpened, { open: openBedAssignment, close: closeBedAssignment }] = useDisclosure(false);
  const [monitorVitalsOpened, { open: openMonitorVitals, close: closeMonitorVitals }] = useDisclosure(false);
  const [equipmentCheckOpened, { open: openEquipmentCheck, close: closeEquipmentCheck }] = useDisclosure(false);
  const [addProtocolOpened, { open: openAddProtocol, close: closeAddProtocol }] = useDisclosure(false);
  ...
  const fetchCases = useCallback(async () => {
    try {
      const response = await emergencyService.getCases({
        ...
        triageLevel: selectedTriage || undefined,
        search: searchQuery || undefined,
      });
      console.log('✅ [DEBUG] Emergency Cases API response:', response);
      const cases = response.data?.items || [];
      setEmergencyCases(cases);
    } catch (err: any) {
      console.warn('Error fetching emergency cases:', err.response?.data?.message || err.message);
      setEmergencyCases([]);
    }
  }, [selectedStatus, selectedTriage, searchQuery]);

  const fetchStats = useCallback(async () => {
    try {
      const response = await emergencyService.getStats();
      console.log('✅ [DEBUG] Emergency Stats API response:', response);
      setEmergencyStats(response.data);
    } catch (err: any) {
      console.warn('Error fetching emergency stats:', err.response?.data?.message || err.message);
      setEmergencyStats({
        total: 0,
        waiting: 0,
        inTreatment: 0,
  ...
  const fetchQueue = useCallback(async () => {
    try {
      ...
      console.log('✅ [DEBUG] Emergency Queue API response:', response);
      setEmergencyQueue(response.data || []);
    } catch (err: any) {
      console.warn('Error fetching emergency queue:', err.response?.data?.message || err.message);
      setEmergencyQueue([]);
    }
  }, []);
  ...
  const fetchAllData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      await Promise.all([fetchCases(), fetchStats(), fetchQueue(), fetchPatients()]);
    } catch (err: any) {
      console.error('Error loading emergency data:', err);
      setError(err.response?.data?.message || err.message || 'Failed to load emergency data');
    } finally {
      setLoading(false);
    }
  }, [fetchCases, fetchStats, fetchQueue, fetchPatients]);
  ...
  }, [searchQuery, selectedTriage, selectedStatus, fetchCases, loading]);

  // Filter emergency cases
  const filteredCases = useMemo(() => {
    return emergencyCases.filter((emergencyCase) => {
      const matchesSearch =
  ...
  }, [emergencyCases, searchQuery, selectedTriage, selectedStatus]);

  // Filter ICU beds - using IPD beds API
  const [icuBeds, setIcuBeds] = useState<any[]>([]);

  const fetchICUBeds = useCallback(async () => {
    try {
      const ipdService = (await import('../../../services/ipd.service')).default;
      const response = await ipdService.getBeds({});
      console.log('✅ [DEBUG] ICU Beds API response:', response);
      const beds = response.data?.items || [];
      console.log('✅ [DEBUG] ICU Beds parsed:', beds.length, 'beds');
      setIcuBeds(beds);
    } catch (err: any) {
      console.warn('Error fetching ICU beds:', err.response?.data?.message || err.message);
      setIcuBeds([]);
    }
  }, []);
  ...
  }, [activeTab, fetchICUBeds]);

  const filteredBeds = useMemo(() => {
    return icuBeds.filter((bed) => {
      const matchesSearch =
  ...
  }, [icuBeds, searchQuery, selectedBedStatus]);
  ...
  const statsCards = [
    {
      ...
      icon: IconUrgent,
      color: 'red',
      trend: `+${emergencyStats?.waiting || 0}`,
    },
    {
      title: 'ICU Beds',
      value: `${icuBeds.filter(b => b.status === 'OCCUPIED').length}/${icuBeds.length}`,
      icon: IconBed,
      color: 'blue',
      trend: `${icuBeds.length > 0 ? Math.round((icuBeds.filter(b => b.status === 'OCCUPIED').length / icuBeds.length) * 100) : 0}% occupied`,
    },
    {
      title: 'Average Wait Time',
      value: `${emergencyStats?.averageWaitTime || 0}min`,
      icon: IconClockHour4,
      color: 'orange',
      trend: '0min',
    },
    {
      ...
      value: emergencyStats?.criticalCases || 0,
      icon: IconAlertTriangle,
      color: 'purple',
      trend: `+${emergencyStats?.criticalCases || 0}`,
    },
  ];

  // Chart data from API
  const triageDistribution = [
    { name: 'Critical', value: emergencyCases.filter(c => c.triageLevel === 'CRITICAL').length, color: 'red.6' },
    { name: 'Urgent', value: emergencyCases.filter(c => c.triageLevel === 'URGENT').length, color: 'orange.6' },
    { name: 'Semi-Urgent', value: emergencyCases.filter(c => c.triageLevel === 'SEMI_URGENT').length, color: 'yellow.6' },
    { name: 'Non-Urgent', value: emergencyCases.filter(c => c.triageLevel === 'NON_URGENT').length, color: 'green.6' },
  ];
  ...
  return (
    <Container size="xl" py={{ base: 'xs', sm: 'sm', md: 'md' }} px={{ base: 'xs', sm: 'sm', md: 'md', lg: 'lg' }}>
            ...
            <SimpleGrid cols={{ base: 1, lg: 2 }} spacing="lg">
              ...
              <Card padding="lg" radius="md" withBorder>
                ...
                {hourlyAdmissions.length === 0 ? (
                  <Text c="dimmed" ta="center" p="xl">No admission data available</Text>
                ) : (
                  <SimpleAreaChart
              ...
              <Card padding="lg" radius="md" withBorder style={{ gridColumn: '1 / -1' }}>
                ...
                {bedOccupancy.length === 0 ? (
                  <Text c="dimmed" ta="center" p="xl">No bed occupancy trend data available</Text>
                ) : (
                  <SimpleLineChart
                    data={bedOccupancy}
                    dataKey="date"
          ...
          <Paper p="md" radius="md" withBorder mt="md">
            ...
            <Stack gap="md">
              {emergencyQueue.length === 0 ? (
                <EmptyState
                  icon={<IconUrgent size={48} />}
                  title="No patients in triage queue"
                  description="Triage queue is empty"
                  size="sm"
                />
              ) : (
        ...
        <Tabs.Panel value="icu">
          <Paper p="md" radius="md" withBorder mt="md">
            <Group justify="space-between" mb="lg">
              ...
              <Group>
                <Button leftSection={<IconBed size={16} />} variant="light" onClick={openBedAssignment}>
                  Bed Assignment
                </Button>
                <Button leftSection={<IconActivity size={16} />} onClick={openMonitorVitals}>Monitor Vitals</Button>
              </Group>
            </Group>
            ...
            <SimpleGrid cols={{ base: 1, md: 2, lg: 3 }} spacing="lg">
              {filteredBeds.length === 0 ? (
                <EmptyState
                  ...
                  title="No ICU beds"
                  description="No ICU beds available or occupied"
                  size="sm"
                />
              ) : (
                ...
                <Card key={bed.id} padding="lg" radius="md" withBorder>
                  ...
                  {bed.patient ? (
                    <Stack gap="sm">
                      <Text size="sm" c="dimmed">Patient assigned to this bed</Text>
                      <Text size="sm">Bed is occupied</Text>
                    </Stack>
                  ) : (
                    <Text size="sm" c="dimmed">Bed is available</Text>
                  )}
        ...
        <Tabs.Panel value="equipment">
          <Paper p="md" radius="md" withBorder mt="md">
            <Group justify="space-between" mb="lg">
              ...
              <Button leftSection={<IconSettings size={16} />} onClick={openEquipmentCheck}>Equipment Check</Button>
            </Group>

            {/* Equipment Grid */}
            <EmptyState
              icon={<IconSettings size={48} />}
              title="Equipment tracking coming soon"
              description="Critical care equipment management will be available soon"
              size="sm"
            />
            <SimpleGrid cols={{ base: 1, md: 2, lg: 3 }} spacing="lg" style={{ display: 'none' }}>
              {[].map((equipment) => (
        ...
        <Tabs.Panel value="protocols">
          <Paper p="md" radius="md" withBorder mt="md">
            <Group justify="space-between" mb="lg">
              ...
              <Button leftSection={<IconPlus size={16} />} onClick={openAddProtocol}>Add Protocol</Button>
            </Group>
            ...
            <EmptyState
              ...
              description="Standard emergency protocols are available in the system"
              size="sm"
            />
            <Stack gap="lg" style={{ display: 'none' }}>
              {[].map((protocol) => (
      ...
      <Modal opened={bedAssignmentOpened} onClose={closeBedAssignment} title="Bed Assignment" size="lg">
        <Stack gap="md">
          <Select
            label="Select Patient"
            placeholder="Choose patient for bed assignment"
            data={patients.map(p => ({
              value: p.id,
              label: `${p.firstName} ${p.lastName} - ${p.patientId || p.medicalRecordNumber || ''}`
            }))}
          ...
          <Select
            ...
            data={icuBeds.filter(b => b.status === 'AVAILABLE').map(b => ({
              value: b.id,
              label: `Bed ${b.bedNumber} - ${b.ward?.name || 'ICU'}`
            }))}
            searchable
            required
          />
          <Select
            ...
            required
          />
          <Textarea label="Notes" placeholder="Enter assignment notes" rows={3} />
          <Group justify="flex-end">
            <Button variant="light" onClick={closeBedAssignment}>Cancel</Button>
            <Button onClick={() => {
              notifications.show({
                title: 'Bed Assigned',
                message: 'Patient has been assigned to the bed successfully',
                color: 'green',
              });
              closeBedAssignment();
            }}>Assign Bed</Button>
      ...
      <Modal opened={monitorVitalsOpened} onClose={closeMonitorVitals} title="Monitor Patient Vitals" size="lg">
        <Stack gap="md">
          <Select
            label="Select Patient"
            placeholder="Choose patient to monitor"
            data={patients.map(p => ({
              value: p.id,
              label: `${p.firstName} ${p.lastName} - ${p.patientId || p.medicalRecordNumber || ''}`
            }))}
            ...
            required
          />
          <Text size="sm" fw={500} mt="md">Record Vital Signs</Text>
          <SimpleGrid cols={2}>
            <NumberInput label="Heart Rate (bpm)" placeholder="72" />
            <TextInput label="Blood Pressure" placeholder="120/80" />
            <NumberInput label="Temperature (°F)" placeholder="98.6" />
          ...
          <Textarea label="Clinical Notes" placeholder="Enter observations" rows={3} />
          <Group justify="flex-end">
            <Button variant="light" onClick={closeMonitorVitals}>Cancel</Button>
            <Button onClick={() => {
              notifications.show({
      ...
      <Modal opened={equipmentCheckOpened} onClose={closeEquipmentCheck} title="Equipment Check" size="lg">
        <Stack gap="md">
          <Text c="dimmed" size="sm">Perform routine equipment inspection and maintenance check</Text>
          <Select
            ...
            required
          />
          <TextInput label="Equipment ID" placeholder="Enter equipment ID" required />
          <Select
            label="Status"
            ...
            required
          />
          <Textarea label="Inspection Notes" placeholder="Enter inspection findings" rows={4} />
          <Group justify="flex-end">
            <Button variant="light" onClick={closeEquipmentCheck}>Cancel</Button>
            <Button onClick={() => {
              notifications.show({
                title: 'Equipment Check Completed',
                message: 'Equipment inspection has been recorded',
                color: 'green',
              });
              closeEquipmentCheck();
            }}>Submit Check</Button>
      ...
      {/* Add Protocol Modal */}
      <Modal opened={addProtocolOpened} onClose={closeAddProtocol} title="Add Emergency Protocol" size="lg">
        <Stack gap="md">
          <TextInput label="Protocol Name" placeholder="Enter protocol name" required />
          <Select
            label="Category"
            ...
            required
          />
          <Textarea label="Description" placeholder="Enter protocol description" rows={3} required />
          <Textarea label="Protocol Steps" placeholder="Enter step-by-step procedures (one per line)" rows={6} required />
          <TextInput label="Version" placeholder="e.g., 1.0" required />
          <Group justify="flex-end">
            <Button variant="light" onClick={closeAddProtocol}>Cancel</Button>
            <Button onClick={() => {
              notifications.show({
                title: 'Protocol Added',
                message: 'Emergency protocol has been added successfully',
                color: 'green',
              });
              closeAddProtocol();
            }}>Add Protocol</Button>
